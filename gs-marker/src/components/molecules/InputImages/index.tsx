...
const InputImages = (props: InputImagesProps) => {
  const { images, maximumNumber, name, hasError, width = '100%', height = '260px', onChange } = props;
  const files = useMemo(
    ...
    <InputImagesContainer flexDirection="column">
      {images &&
        images.map((img, index) => {
          return <ImagePreview alt={img.id} key={index} src={img.src} height={height} onRemove={onRemove} />;
        })}
